...
impl<Tok> PreprocessedGrammar<Tok> {
  pub fn build_dot_graph(self) -> gv::GraphBuilder {
     ...
     * states. */
    {
      ...
      for (this_ref, node) in trie_node_universe.into_iter().enumerate() {
        let this_vertex = gv::Vertex {
          id: gv::Id::validate(format!("stack_trie_node_{}", this_ref)),
      ...
      for (this_id, (vtx, node_ref)) in vertex_mapping.into_iter().enumerate() {
        let this_id = gv::Id::validate(format!("cyclic_epsilon_graph_vertex_{}", this_id));
        let this_vertex = gv::Vertex {
          id: this_id.clone(),
    ...
    let border_vertices = gv::Subgraph {
      id: gv::Id::validate("border_vertices".to_string()),
    ...
    let state_vertices = gv::Subgraph {
      id: gv::Id::validate("state_vertices".to_string()),
