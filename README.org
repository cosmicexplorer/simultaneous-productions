...
** code TODO [0/6]
1. [ ] graphviz for every intermediate graph in grammar processing!
2. [ ] implement typed callbacks (for e.g. AST construction) by stuffing intermediate arguments in the stack symbols!
3. [ ] demonstrate a grammar generator for regex constructs!
4. [ ] implement context-sensitive and recursively-enumerable grammatical constructs!
  - also consider backrefs (can we make these sub-exponential?)!
5. [ ] make a single iteration of parsing/resolution into a re-entrant(/lock-free?) process!
...
6. [ ] generate intermediate grammar graphs with proptest (maybe grammars themselves with cargo-fuzz too?)!
